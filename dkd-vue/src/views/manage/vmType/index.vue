...
const data = reactive({
  ...
  rules: {
    ...
    image: [{ required: true, message: "设备图片不能为空", trigger: "blur" }],
    vmRow: [{ required: true, message: "货道行列不能为空", trigger: "blur" }],
    vmCol: [{ required: true, message: "货道列不能为空", trigger: "blur" }],
    channelMaxCapacity: [
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除设备类型管理编号为"' + _ids + '"的数据项？')
    .then(function () {

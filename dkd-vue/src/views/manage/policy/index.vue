  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="detailOpen" width="800px" append-to-body>
      ...
      <el-table :data="vmList">
        ...
        <el-table-column prop="addr" label="点位地址" align="center" />
...
const vmList = ref([]);
async function getVmInfo(row) {
  const policyId = row.policyId;
  //策略名称
  form.value = row;

  //包含设备
  const vmListRes = await listVm({ ...loadAllParams, policyId });
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除策略管理编号为"' + _policyIds + '"的数据项？')
    .then(function () {

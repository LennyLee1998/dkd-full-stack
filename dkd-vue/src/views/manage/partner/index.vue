<template>
  <div class="app-container">
      ...
      >
        <template #default="scope">
          ...
          <el-button
            link
            type="primary"
            @click="getPartnerInfo(scope.row)"
    ...
    >
      <el-row>
        <el-col :span="12"> 合作商名称: {{ partnerInfo.partnerName }} </el-col>
        <el-col :span="12"> 联系人: {{ partnerInfo.contactPerson }} </el-col>
      </el-row>
      <el-row>
        <el-col :span="12"> 联系电话: {{ partnerInfo.contactPhone }} </el-col>
        <el-col :span="12"> 分成比例: {{ partnerInfo.profitRatio }} </el-col>
      </el-row>
    </el-dialog>
  </div>
</template>
...
function handleUpdate(row) {
  ...
  getPartner(_id).then((response) => {
    response.data.profitRatio = response.data.profitRatio + "%";
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除合作商管理编号为"' + _ids + '"的数据项？')
    .then(function () {
...
function resetPwd(row) {
  const _ids = row.id;
  proxy.$modal
    .confirm('你确定要重置该合作商密码吗？')
    .then(function () {
      return updatePwd(_ids);
    })
    .then(() => {

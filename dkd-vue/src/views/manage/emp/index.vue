    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="empRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="角色" prop="roleId">
          <!-- <el-input v-model="form.roleId" placeholder="请选择角色" /> -->
...
async function getRoleAndRegionList() {
  console.log("aa");
  const roleListRes = await listRole(loadAllParams);
  roleList.value = roleListRes.rows.map((role) => ({
    value: role.roleId,
    label: role.roleName,
  }));

  const regionListRes = await listRegion(loadAllParams);
  regionList.value = regionListRes.rows.map((region) => ({

<template>
  <div class="app-container">
    ...
    >
      <el-form-item label="区域名称" prop="regionName">
        <el-input v-model="regionName" disabled />
      </el-form-item>
      <label>包含点位: </label>
...
const { regionName, nodeList } = toRefs(regionInfo);
const infoOpen = ref(false);
async function getRegionInfo(row) {
  reset();
  //获取当前的id
  const _ids = row.id;
  //获取详情
  const regionRes = await getRegion(_ids);
  regionName.value = regionRes.data.regionName;
  const nodeListRes = await listNode({ ...loadAllParams, regionId: _ids });

  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="区域搜索" prop="regionId">
        <el-select v-model="queryParams.regionId" placeholder="请选择区域">
          <el-option
            v-for="region in regionList"
            :key="region.value"
            :label="region.label"
            :value="parseInt(region.value)"
          ></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
      ...
      <el-form ref="nodeRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="商圈类型" prop="businessType">
          <el-select v-model="form.businessType" placeholder="请选择商圈类型">
            <el-option
              v-for="dict in node_business_type"
              :key="dict.value"
              :label="dict.label"
...
function getRegionList() {
  listRegion(loadAllParams).then((response) => {
    regionList.value = response.rows.map((region) => ({
...
function getPartnerList() {
  listPartner(loadAllParams).then((response) => {
    partnerList.value = response.rows.map((partner) => ({
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除点位管理编号为"' + _ids + '"的数据项？')
    .then(function () {
      return delNode(_ids);
    })
    .then(() => {
